...
def get_random_name():
    ...
    last_name = names[(names['country']==c) & (names['type']=='s')].sample(1).iloc[0]['name']

    return (first_name, last_name)

def replace_name(data_tuple, first_name, last_name):
    text = data_tuple[0]
    entities = sorted(copy.deepcopy(data_tuple[1]['entities']), key=lambda x:x[0])
...
#n_iter: no. of iterations to train the model
    ...
    with nlp.disable_pipes(*other_pipes) and warnings.catch_warnings():
        ...
        for itn in range(n_iter):
            ...
            send_output(f'Training on {len(TRAIN_DATA)} records, validating on {len(VALID_DATA)} records.')
